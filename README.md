...
- [ ] **Per-fork exact gas schedule** — give the gas schedule the per-fork branching it lacks
  ...
  - **Wiring it up is not a substitution.** The two tables agree on every opcode's *total* but not on how the total is composed: `eth_evm:base_cost/1` prices a warm access at 100 and adds the cold surcharge in the handler, while `eth_fork_schedule:access_cost/3` returns 2600/2100 as one figure. Swapping one for the other would charge a cold `BALANCE` 5100 instead of 2600 and a cold `CALL` 5200 instead of 2600, because the surcharges would be applied twice. Whoever does the wiring has to choose one owner of the composition and change the other side
  ...
  - Fixed: EIP-3860's init-code word cost inside `CREATE`/`CREATE2` — `CREATE` now pays 2 a word and `CREATE2` 8, where before `CREATE` paid nothing and `CREATE2` paid only the 6 for hashing. No double charge: a creation transaction's `data` is run directly by `eth_block:execute_transactions/5` and never enters `do_create/3`, so the intrinsic-gas term and the opcode term cover different things
